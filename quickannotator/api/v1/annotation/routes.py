...
class Annotation(MethodView):
    ...
    def get(self, args, image_id, annotation_class_id):
        ...
        store = AnnotationStore(
            ...
            annotation_class_id,
            args["is_gt"],
            in_work_mag=in_work_mag,
            simplify_tolerance=args.get("simplify_tolerance", 0.0),
        )
    ...
    @bp.alt_response(
        400,
        description="Bad Request: Annotations cannot be saved in tiles that do not intersect the mask.",
    )
    def post(self, args, image_id, annotation_class_id):
        ...
        store = AnnotationStore(
            image_id, annotation_class_id, is_gt=is_gt, in_work_mag=in_work_mag
        )
        ...
        tile_ids_intersecting_mask, _, _ = tilestore.get_tile_ids_intersecting_mask(
            image_id, annotation_class_id
        )
        polygon_tile_ids = {ann.tile_id for ann in anns}
        if annotation_class_id != constants.MASK_CLASS_ID:
            non_intersecting_tile_ids = polygon_tile_ids - set(
                tile_ids_intersecting_mask
            )
            if (
                non_intersecting_tile_ids
            ):  # If there are any tile IDs that do not intersect the mask
                error_message = f"Annotations cannot be saved in tiles that do not intersect the mask: {non_intersecting_tile_ids}"
                [
                    store.delete_annotations_by_tile(tile_id)
                    for tile_id in non_intersecting_tile_ids
                ]  # db_session.rollback() does not prevent the annotations from being saved, so we delete them manually. TODO: proactively check polygons before saving them.
        ...
        for tile_id in polygon_tile_ids:
            mask_cache_manager.invalidate(
                CacheableMask.get_key(image_id, annotation_class_id, tile_id)
            )
    ...
    def put(self, args, image_id, annotation_class_id):
        ...
        store = AnnotationStore(
            image_id, annotation_class_id, args["is_gt"], in_work_mag=in_work_mag
        )
        ...
        if ann is not None:  # Only invalidate if the tile_id is set
            mask_cache_manager.invalidate(
                CacheableMask.get_key(image_id, annotation_class_id, ann.tile_id)
            )
    ...
    def delete(self, args, image_id, annotation_class_id):
        """delete one or more annotations by their IDs"""
        store = AnnotationStore(image_id, annotation_class_id, args["is_gt"])

        # Get annotations for cache invalidation before deleting
        annotations = store.get_annotations_by_ids(args["annotation_ids"])
        tile_ids = {ann.tile_id for ann in annotations}

        result = store.delete_annotations(args["annotation_ids"])

        # Invalidate the mask cache for affected tiles
...
class AnnotationOperation(MethodView):
    ...
    def post(self, args):
        ...
        resp = {
            field: args[field]
            for field in server_models.AnnRespSchema().fields.keys()
            if field in args
        }  # Basically a copy of args without "polygon2" or "operation"
        try:
...
class ExportAnnotationsToServer(MethodView):
    ...
    def post(self, args):
        ...
        is_gt = True
        project_id = (
            get_image_by_id(image_ids[0]).project_id
        )  # NOTE: This is only used for naming the actor, so it's not critical.
        ...
        filepaths = [
            ...
            for image_id, annotation_class_id in list(
                product(image_ids, annotation_class_ids)
            )
        ]

        actor_name = compute_actor_name(
            project_id, constants.NamedRayActorType.ANNOTATION_EXPORTER
        )
        exporter = AnnotationExporter.options(name=actor_name).remote(
            image_ids, annotation_class_ids
        )
