...
import { Image, Project, Annotation, AnnotationClass } from "../types.ts";
...
// Fetch annotations
export const fetchAnnotations = async (image_id: number, annotation_class_id: number, is_gt: boolean) => {
    return await get<Annotation[]>(`/annotation/${image_id}/${annotation_class_id}?is_gt=${is_gt}`);
}

export const fetchAnnotationClasses = async () => {
    return await get<AnnotationClass[]>('/annotation_class/');
}

export const fetchAnnotationClassById = async (annotation_class_id: number) => {
    return await get<AnnotationClass>(`/annotation_class/${annotation_class_id}`);
}
...
export const searchTiles = async (image_id: number, annotation_class_id: number, x1: number, y1: number, x2: number, y2: number) => {
    ...
    return await get(`/tile/${image_id}/${annotation_class_id}/search?${query}`);
}